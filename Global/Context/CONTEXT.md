...
*   **Service Worker Update Cycle:** An old service worker with an aggressive "cache-first" strategy can cache itself and prevent new versions from being installed promptly, leading to users seeing stale content. The correct update procedure is:
    1.  Implement a "network-first" strategy for the app shell.
    2.  **Always increment the `CACHE_NAME` version number** (e.g., `v5` to `v6`) in `service-worker.js` with every logic change. This is the critical step that signals to the browser that an update is available.
    3.  The new service worker will then install, and its `activate` event will delete the old cache. The use of `self.skipWaiting()` and `self.clients.claim()` ensures this transition happens immediately for the user.
...
*   **Modular Script Pathing:** When a global script needs to load its own assets (e.g., SVGs, JSON), using hardcoded relative paths will fail when that script is included in a nested HTML file (e.g., `Tools/Category/Tool/index.html`). The most robust solution is to pass the relative base path from the HTML file to the script via a `data-` attribute on the `<script>` tag (e.g., `data-base-path="../../../"`). The script can then read this `basePath` from `document.currentScript.dataset.basePath` and prepend it to all asset URLs, ensuring they resolve correctly from any directory depth.
